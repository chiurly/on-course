package oncourse.oncourse.fxcontroller;

import javafx.fxml.FXML;
import javafx.fxml.Initializable;
import javafx.scene.control.Alert;
import javafx.scene.control.DatePicker;
import javafx.scene.control.TextArea;
import javafx.scene.control.TextField;
import javafx.stage.Stage;
import oncourse.oncourse.ds.Course;
import oncourse.oncourse.hibernatecontroller.HibernateCourse;

import javax.persistence.EntityManagerFactory;
import javax.persistence.Persistence;
import java.net.URL;
import java.util.ResourceBundle;

public class EditCourseController implements Initializable {

    @FXML
    private TextField idField;
    @FXML
    private TextField nameField;
    @FXML
    private TextArea descriptionArea;
    @FXML
    private DatePicker endDatePicker;
    @FXML
    private DatePicker dateCreatedPicker;

    private final EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("on_course");
    private final HibernateCourse hibernateCourse = new HibernateCourse(entityManagerFactory);

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        Course selectedCourse = hibernateCourse.get(MainController.getSelectedCourseId());
        idField.setText(String.valueOf(selectedCourse.getId()));
        nameField.setText(selectedCourse.getName());
        descriptionArea.setText(selectedCourse.getDescription());
        endDatePicker.setValue(selectedCourse.getEndDate());
        dateCreatedPicker.setValue(selectedCourse.getDateCreated());
    }

    @FXML
    protected void onUpdateButtonClick() {
        if (nameField.getText().isBlank()) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Required fields cannot be empty");
            alert.setContentText("Field \"Name\" cannot be empty");
            alert.showAndWait();
            return;
        }

        Course course = hibernateCourse.get(MainController.getSelectedCourseId());
        course.setName(nameField.getText());
        course.setDescription(descriptionArea.getText());
        course.setEndDate(endDatePicker.getValue());

        hibernateCourse.edit(course);
        closeWindow();
    }

    @FXML
    protected void onDeleteButtonClick() {
        hibernateCourse.remove(MainController.getSelectedCourseId());
        closeWindow();
    }

    @FXML
    protected void onCancelButtonClick() {
        closeWindow();
    }

    private void closeWindow() {
        Stage stage = (Stage) idField.getScene().getWindow();
        stage.close();
    }

}
